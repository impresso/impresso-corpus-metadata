    ...
    for stg, incl_enrich_stats in ENRICHMENT_STAGES.items():
        ...
        for proc in proc_for_stg:
            ...
            if proc["manifest_s3_path"] is None:
                ...
                print(msg)
                # logger.info(msg)
                enrichment["statistics"] = "MISSING MANIFEST!"
                ...
                for ov_sts in manifest_stg["overall_statistics"]:
                    if ov_sts["stage"] == stg.value:
                        for source_type in actual_src_tp_stats:
                            stats_for_enrich[source_type] = {
                                s: ov_sts[source_type][s] for s in incl_enrich_stats
                            }

                # TODO change fd into number of keys??
                enrichment["statistics"] = stats_for_enrich

            if proc["processing_label"] in enrichments_dict:
                enrichments_dict[proc["processing_label"]].append(enrichment)
            else:
                enrichments_dict[proc["processing_label"]] = [enrichment]

    return enrichments_dict
